<template>
	<div>
		...
		<el-dialog :visible.sync="dialogVisible" width="400px" append-to-body>
		...
		setFileList(fileList) {
			...
			fileList.forEach(function(item, index) {
				...
				if (!rawUrl.startsWith("http")) {
					displayUrl = _this.$base.url + rawUrl.replace(/^\/+/, '')
				}
				var name = item.name;
				var file = {
					name: name,
				...
				fileArray.push(file);
				fileUrlArray.push(rawUrl.startsWith("http") ? rawUrl : rawUrl.replace(/^\/+/, ''));
			});
