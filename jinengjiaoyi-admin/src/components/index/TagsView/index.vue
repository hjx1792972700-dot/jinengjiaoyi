		...
		<scroll-pane ref="scrollPane" class="tags-view-wrapper">
			<div class="tags-view-box" :style='{"width":"100%","whiteSpace":"nowrap","position":"relative","background":"#fff"}'>
				<router-link
					...
					<el-icon v-if="!tag.meta?.affix" class="close-icon" @click.prevent.stop="closeSelectedTag(tag)"><Close /></el-icon>
				</router-link>
			</div>
		</scroll-pane>
	...
	export default {
		...
		methods: {
			...
			moveToCurrentTag() {
				...
				const tagRefs = tags ? (Array.isArray(tags) ? tags : [tags]) : []
				this.$nextTick(() => {
					for (const tag of tagRefs) {
						const resolved = this.$router.resolve(tag.$props.to)
