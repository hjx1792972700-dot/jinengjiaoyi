	...
	<div class="main-content" style="min-height:100%;width:100%;padding:0;background:none;">
		...
		<el-table-column :resizable='true' prop="yonghuxingming"
			...
			<el-table-column :resizable='true' prop="gongjirenxingming"
				label="技能发布者" min-width="100">
				<template #default="scope">
					<div>{{scope.row.gongjirenxingming}}</div>
				</template>
			</el-table-column>
			<el-table-column :resizable='true'
				label="申请人技能" min-width="100">
				<template #default="scope">
					{{scope.row.shenqingrentigong || scope.row.jinengfenlei || '--'}}
				</template>
			</el-table-column>
			<el-table-column :resizable='true'
				label="发布者技能" min-width="100">
				<template #default="scope">
					{{scope.row.fabuzhetigong || scope.row.biaoti || '--'}}
				</template>
			</el-table-column>
				<el-table-column :resizable='true' prop="jiaohuanshijian"
					label="交换时间" min-width="110">
					<template #default="scope">
		...
		<template v-if="statType">
			<div class="statShow1">
				<div class="echarts1" v-if="isAuth('jiaohuanjilu','交换量统计')">
					<div id="jiaohuanshijianChart1" style=" width: 100%;height: 100%"></div>
				</div>
			</div>
		</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			pingjiafankuiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				...
				this.pingjiafankuiCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','jiaohuanjilu');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.pingjiafankuiCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.pingjiafankuiCrossaddOrUpdate.init(row.id,type);
				});
			},
			changeStatQuery(arr) {
				if(arr.length==1) {
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http({
						url: "jiaohuanjilu/group/jiaohuanshijian",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
