		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'"  label="分类名称" prop="typename" >
					<el-select :disabled="ro.typename" v-model="ruleForm.typename" placeholder="请选择分类名称"  filterable>
						<el-option
		...
		methods: {
			...
			async onSubmit() {
					if(this.ruleForm.picture!=null) {
						this.ruleForm.picture = this.ruleForm.picture.split(',').map(u => u.replace(/^\/+/, '')).join(',');
					}
					if(this.ruleForm.headportrait!=null) {
						this.ruleForm.headportrait = this.ruleForm.headportrait.split(',').map(u => u.replace(/^\/+/, '')).join(',');
					}
