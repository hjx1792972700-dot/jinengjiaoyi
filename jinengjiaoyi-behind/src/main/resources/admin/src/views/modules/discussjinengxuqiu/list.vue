		...
		<template v-if="showFlag ">
			...
			<div class="admin-actions">
				<div class="action-left">
					...
					<el-button class="btn-danger" v-if="isAuth('discussjinengxuqiu','删除')" :disabled="dataListSelections.length?false:true" size="small" @click="deleteHandler()"><template #icon><Delete /></template>删除</el-button>
				</div>
			</div>
			<div class="admin-table-wrap">
				...
				<el-table-column :resizable='true' :sortable='true'
					...
					<el-table-column width="300" label="操作">
						<template #default="scope">
							<el-button v-if=" isAuth('discussjinengxuqiu','修改') " type="warning" plain size="small" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
							<el-button v-if="isAuth('discussjinengxuqiu','查看评论')" type="success" plain size="small" @click="disscussListHandler(scope.row.id)">查看评论</el-button>
							<el-button type="info" plain size="small" @click="addOrUpdateHandler(scope.row.id)">回复</el-button>
							<el-button v-if="isAuth('jinengxuqiu','删除评论')" type="danger" plain size="small" @click="deleteHandler(scope.row.id ,scope.row.refid)">删除</el-button>
						</template>
					</el-table-column>
				</el-table>
			</div>
...
<script>
	...
	import { Back, RefreshRight, Delete } from '@element-plus/icons-vue';
	export default {
		data() {
			return {
				indexQueryCondition: '',
		...
		methods: {
			...
			async deleteHandler(id ,refid ) {
				...
				await this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
					...
					await this.$http({
						url: "discussjinengxuqiu/delete",
						method: "post",
						data: ids
					}).then(async ({ data }) => {
						if (data && data.code === 0) {
							this.$http({
								url: 'jinengxuqiu/info/' + refid,
								method: "get",
							}).then(res=>{
								if(res.data&&res.data.code==0){
									res.data.data.discussnum = res.data.data.discussnum - Number(ids.length)
									this.$http({
										url: 'jinengxuqiu/update',
										method: "post",
										data:res.data.data
									}).then(res1=>{
