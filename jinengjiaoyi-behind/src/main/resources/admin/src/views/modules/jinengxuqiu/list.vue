	...
	<div class="main-content" style="min-height:100%;width:100%;padding:0;background:none;">
		<template v-if="showFlag && !statType">
			...
			<el-table-column prop="yonghuxingming" label="用户" min-width="110">
				...
				<template #default="scope">
					<div class="heat-row"><el-icon><View /></el-icon> {{scope.row.clicknum||0}}</div>
					<div class="heat-row"><el-icon><StarFilled /></el-icon> {{scope.row.storeupnum||0}}</div>
				</template>
			</el-table-column>
		...
		<el-dialog :title="this.batchIds.length>1?'批量审核':'审核'" v-model="sfshBatchVisiable" width="480px" :close-on-click-modal="false">
			<el-form ref="shBatchForm" :model="shBatchForm" :rules="shRules" label-width="80px">
				<el-form-item label="审核状态" prop="sfsh">
					<el-select v-model="shBatchForm.sfsh" placeholder="请选择审核状态" style="width:100%">
				...
				<el-form-item label="回复内容" prop="shhf">
					<el-input type="textarea" :rows="5" v-model="shBatchForm.shhf" placeholder="请输入审核回复内容"></el-input>
				</el-form-item>
			</el-form>
			<template #footer>
				<el-button class="btn-plain" size="small" @click="sfshBatchVisiable=false">取消</el-button>
				<el-button class="btn-primary" size="small" @click="shBatchHandler">确认提交</el-button>
			</template>
		</el-dialog>
		...
		<template v-if="statType">
			<div class="statShow2">
				<div class="echarts1" v-if="isAuth('jinengxuqiu','技能需求类型')">
					<el-select 
						v-if="changeStatQuery(['users'])"
						v-model="chartQuery1.jinengfenlei" placeholder="请选择" @change="chartDialog1" clearable>
						<el-option v-for="item in jinengfenleiChartOptions1" :label="item" :value="item"></el-option>
					</el-select>
					<div id="jinengfenleiChart1" style="width: 100%;height: calc(100% - 50px) "></div>
				</div>
				<div class="echarts2" v-if="isAuth('jinengxuqiu','每日需求量统计')">
					<div id="fabushijianChart2" style=" width: 100%;height: 100%"></div>
				</div>
			</div>
		</template>
		...
		<el-dialog v-model="chatVisible" @close="clearChat" :title="nowname" :append-to-body="true">
			...
			<template #footer>
				...
				<div style="display: flex;align-items: center">
					<el-input @keydown.enter="addChat(null)" v-model="chatForm.content" placeholder="请输入内容"
...
<script>
	...
	import jiaohuanshenqingCrossAddOrUpdate from "../jiaohuanshenqing/add-or-update.vue";
	...
	methods: {
		statChartClick() {
				...
				if(this.isAuth('jinengxuqiu','每日需求量统计')) {
			...
			chatClick(row) {
				...
				this.$http.get(`${chatUser}/query`, {
					...
					if (res.data && res.data.code == 0) {
						...
						this.nowname = res.data.data.yonghuxingming || res.data.data[chatUserName2?chatUserName2:chatUserName]
			...
			jiaohuanshenqingCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				let username = this.$storage.get("adminName")
				if(row.yonghuzhanghao == username) {
					this.$message.error('自己不能自己供给');
					return false
				}
				...
				this.jiaohuanshenqingCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','jinengxuqiu');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.jiaohuanshenqingCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.jiaohuanshenqingCrossaddOrUpdate.init(row.id,type);
				});
			},
			changeStatQuery(arr) {
				if(arr.length==1) {
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					this.$http({
						url: "jinengxuqiu/group/jinengfenlei",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
			...
			chartDialog2() {
				this.$nextTick(()=>{
					...
					this.$http({
						url: "jinengxuqiu/group/fabushijian",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussjinengxuqiu',query:{refid:id}});
			},
			//批量审核窗口
			shBatchDialog(){
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "jinengxuqiu/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
