<template>
	<div class="main-content" style="min-height:100%;width:100%;padding:0;background:none;">
			...
			<el-table-column prop="fengmian" width="70" label="封面" align="center">
					<template #default="scope">
						<div v-if="scope.row.fengmian">
							<img v-if="scope.row.fengmian.substring(0,4)=='http'" :src="scope.row.fengmian.split(',')[0]" width="48" height="48" style="object-fit:cover;border-radius:6px;cursor:pointer" @click="previewImage(scope.row.fengmian.split(',')[0])">
							<img v-else :src="$base.url+scope.row.fengmian.split(',')[0]" width="48" height="48" style="object-fit:cover;border-radius:6px;cursor:pointer" @click="previewImage($base.url+scope.row.fengmian.split(',')[0])">
						</div>
						<div v-else style="color:#475569;font-size:11px;">-</div>
					</template>
				</el-table-column>
				<el-table-column prop="xuqiubiaoti" label="技能名称" min-width="140" show-overflow-tooltip>
					<template #default="scope">
				...
				<el-table-column prop="yonghuxingming" label="用户" min-width="100">
		...
		<!-- 审核对话框 -->
		<el-dialog :title="auditDialogTitle" v-model="auditDialogVisible" width="480px" :close-on-click-modal="false" top="30vh">
			<el-form ref="auditForm" :model="auditForm" :rules="auditRules" label-width="80px">
				...
				<el-form-item label="审核回复" prop="shhf">
					<el-input type="textarea" :rows="3" v-model="auditForm.shhf" :placeholder="auditForm.sfsh === '否' ? '请填写拒绝原因' : '选填'"></el-input>
				</el-form-item>
			</el-form>
			<template #footer>
		...
		<!-- 详情查看对话框 -->
		<el-dialog title="技能详情" v-model="detailDialogVisible" width="680px" top="8vh">
			<div v-if="detailData">
				<el-form label-width="100px">
					<el-form-item label="技能名称">{{detailData.xuqiubiaoti}}</el-form-item>
					<el-form-item label="技能分类">{{detailData.jinengfenlei}}</el-form-item>
					<el-form-item label="费用">
						<span v-if="detailData.xuqiufeiyong">¥{{detailData.xuqiufeiyong}}</span>
						<span v-else>面议</span>
					</el-form-item>
					<el-form-item label="用户">{{detailData.yonghuxingming}}</el-form-item>
					<el-form-item label="手机号" v-if="detailData.shoujihao">{{detailData.shoujihao}}</el-form-item>
					<el-form-item label="信誉指数">{{detailData.xinyuzhishu || '-'}}</el-form-item>
					...
					<el-form-item label="技能描述">{{detailData.xuqiumiaoshu || '暂无'}}</el-form-item>
					<el-form-item label="技能详情" v-if="detailData.xuqiuxiangqing">
						<div v-html="detailData.xuqiuxiangqing" class="ql-snow ql-editor" style="padding:0;max-height:300px;overflow-y:auto;"></div>
					</el-form-item>
					<el-form-item label="封面" v-if="detailData.fengmian">
						<img v-if="detailData.fengmian.substring(0,4)=='http'" :src="detailData.fengmian.split(',')[0]" style="max-width:200px;border-radius:6px;">
						<img v-else :src="$base.url+detailData.fengmian.split(',')[0]" style="max-width:200px;border-radius:6px;">
					</el-form-item>
				</el-form>
			</div>
			<template #footer>
