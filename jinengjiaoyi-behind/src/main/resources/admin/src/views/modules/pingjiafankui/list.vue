...
<template>
	<div class="main-content" style="min-height:100%;width:100%;padding:0;background:none;">
		...
		<template v-if="showFlag  &&!statType ">
			<div class="admin-search">
				<div class="search-field">
					...
					<el-input v-model="searchForm.biaoti" placeholder="标题" @keydown.enter="search()" clearable size="small"></el-input>
				</div>
			...
			<div class="admin-table-wrap">
				...
				<el-table-column :resizable='true' prop="huifutupian" width="80" label="图片">
					<template #default="scope">
						<div v-if="scope.row.huifutupian">
							...
							<img v-else-if="scope.row.huifutupian.substring(0,4)=='http'" :src="scope.row.huifutupian.split(',')[0]" width="56" height="56" style="object-fit: cover;border-radius:6px;cursor:pointer" @click="imgPreView(scope.row.huifutupian.split(',')[0])">
							<img v-else :src="$base.url+scope.row.huifutupian.split(',')[0]" width="56" height="56" style="object-fit: cover;border-radius:6px;cursor:pointer" @click="imgPreView($base.url+scope.row.huifutupian.split(',')[0])">
						</div>
						<div v-else style="color:#64748b;font-size:12px;">无图片</div>
					</template>
				</el-table-column>
				<el-table-column :resizable='true'											prop="biaoti"
					label="标题" min-width="120" show-overflow-tooltip>
					<template #default="scope">
						{{scope.row.biaoti}}
					</template>
				</el-table-column>
				...
				<el-table-column :resizable='true'											prop="jiaohuanjineng"
					label="交换技能" min-width="90">
					<template #default="scope">
						{{scope.row.jiaohuanjineng}}
					</template>
				</el-table-column>
			<el-table-column :resizable='true'										prop="yonghuxingming"
				label="用户" min-width="110">
				<template #default="scope">
					<div>{{scope.row.yonghuxingming}}</div>
				</template>
			</el-table-column>
			<el-table-column :resizable='true'										prop="jiaohuanrenxingming"
				label="交换人" min-width="110">
				<template #default="scope">
					<div>{{scope.row.jiaohuanrenxingming}}</div>
				</template>
			</el-table-column>
				<el-table-column :resizable='true'											prop="pingjiajieguo"
					label="评价" min-width="70" align="center">
					<template #default="scope">
					<span class="rating-badge rating-good" v-if="scope.row.pingjiajieguo=='好评'">好评</span>
					<span class="rating-badge rating-bad" v-else-if="scope.row.pingjiajieguo=='差评'">差评</span>
					<span class="rating-badge rating-normal" v-else-if="scope.row.pingjiajieguo=='一般'">一般</span>
						<span v-else style="color:#64748b">{{scope.row.pingjiajieguo||'--'}}</span>
					</template>
				</el-table-column>
				<el-table-column :resizable='true'											prop="pingjiashijian"
					label="评价时间" min-width="110">
					<template #default="scope">
						{{scope.row.pingjiashijian}}
					</template>
				</el-table-column>
				<el-table-column :resizable='true' label="资料" min-width="90" align="center">
					<template #default="scope">
						<el-button v-if="scope.row.huifushipin" type="text" size="mini" @click="preClick(scope.row.huifushipin)">视频</el-button>
						<el-button v-if="scope.row.huifufujian" type="text" size="mini" @click="download(scope.row.huifufujian)">附件</el-button>
						<span v-if="!scope.row.huifushipin && !scope.row.huifufujian" style="color:#64748b;font-size:12px;">无</span>
					</template>
				</el-table-column>
					<el-table-column width="100" label="操作">
						<template #default="scope">
							<el-button size="mini" v-if="isAuth('pingjiafankui','查看')" type="success" plain @click="addOrUpdateHandler(scope.row.id,'info')">查看</el-button>
						</template>
					</el-table-column>
				</el-table>
			</div>
		...
		<el-dialog :title="this.batchIds.length>1?'批量审核':'审核'" v-model="sfshBatchVisiable" width="480px" :close-on-click-modal="false">
		...
		<template v-if="statType">
			<div class="statShow1">
				<div class="echarts1" v-if="isAuth('pingjiafankui','评价统计')">
					<div id="pingjiajieguoChart1" style=" width: 100%;height: 100%"></div>
				</div>
			</div>
		</template>
	...
	export default {
		data() {
			return {
				...
				gauge: {"tooltip":{"backgroundColor":"#123","textStyle":{"color":"#333"}},"backgroundColor":"transparent","color":["#8838FF","#e67d29","#50DFAC","#BD8CE8","#C500FF","#FD0B6A","#9ADBC4","#FFAA00","#000000"],"title":{"top":"top","left":"left","textStyle":{"fontSize":14,"lineHeight":24,"color":"#333","fontWeight":600}},"series":{"pointer":{"offsetCenter":[0,"10%"],"icon":"path://M2.9,0.7L2.9,0.7c1.4,0,2.6,1.2,2.6,2.6v115c0,1.4-1.2,2.6-2.6,2.6l0,0c-1.4,0-2.6-1.2-2.6-2.6V3.3C0.3,1.9,1.4,0.7,2.9,0.7z","width":8,"length":"80%"},"axisLine":{"lineStyle":{"shadowOffsetX":0,"shadowOffsetY":0,"opacity":0.5,"shadowBlur":1,"shadowColor":"#000"},"roundCap":true},"anchor":{"show":true,"itemStyle":{"color":"inherit"},"size":18,"showAbove":true},"emphasis":{"disabled":false},"progress":{"show":true,"roundCap":true,"overlap":true},"splitNumber":25,"detail":{"formatter":"{value}","backgroundColor":"inherit","color":"#fff","borderRadius":3,"width":20,"fontSize":12,"height":10},"title":{"fontSize":14},"animation":true}},
		...
		methods: {
			...
			chartDialog1() {
				this.$nextTick(()=>{
					var pingjiajieguoChart1 = echarts.init(document.getElementById("pingjiajieguoChart1"),'macarons');
					let params = {}
					...
					this.$http({
						url: "pingjiafankui/group/pingjiajieguo",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
							...
							for(let i=0;i<res.length;i++){
								...
								// 统计图设置对了吗
								xAxis.push(res[i].pingjiajieguo);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].pingjiajieguo
								})
							}
							var option = {};
							...
							// 使用刚指定的配置项和数据显示图表。
							pingjiajieguoChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								pingjiajieguoChart1.resize();
							};
						}
					})
				})
			...
			getDataList() {
				...
				if(this.searchForm.jiaohuanjineng!='' && this.searchForm.jiaohuanjineng!=undefined){
					params['jiaohuanjineng'] = '%' + this.searchForm.jiaohuanjineng + '%'
				}
				if(this.searchForm.jiaohuanrenxingming!='' && this.searchForm.jiaohuanrenxingming!=undefined){
					params['jiaohuanrenxingming'] = '%' + this.searchForm.jiaohuanrenxingming + '%'
				}
				if(this.searchForm.pingjiajieguo!='' && this.searchForm.pingjiajieguo!=undefined){
					params['pingjiajieguo'] = this.searchForm.pingjiajieguo
				}
				...
				this.$refs["shBatchForm"].validate(valid => {
					if(valid){
						this.$confirm(`是否${this.batchIds.length>1?'一键审核':'审核'}选中数据?`, "提示", {
							...
							this.$http({
								url: "pingjiafankui/shBatch?sfsh="+this.shBatchForm.sfsh+"&shhf="+this.shBatchForm.shhf,
								method: "post",
								data: this.batchIds
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
...
.rating-good { background: rgba(34,197,94,0.15); color: #4ade80; border: 1px solid rgba(34,197,94,0.3); }
.rating-bad { background: rgba(239,68,68,0.15); color: #f87171; border: 1px solid rgba(239,68,68,0.3); }
.rating-normal { background: rgba(245,158,11,0.15); color: #fbbf24; border: 1px solid rgba(245,158,11,0.3); }
...
.audit-fail { background: rgba(239,68,68,0.15); color: #f87171; border: 1px solid rgba(239,68,68,0.3); }
	...
	.statShow1 {
		...
		.echarts1 {
			...
			box-shadow: none;
			margin: 10px;
			background: url(http://codegen.caihongy.cn/20251013/f0b407eb488c42b593b6763f972bb66f.png) no-repeat left center / auto 100%,url(http://codegen.caihongy.cn/20251013/66d53461fe624233beaf7244300ce11b.png) no-repeat right center / auto 100%,url(http://codegen.caihongy.cn/20251013/ca053e0287864aa0afe240c4cf8d40ef.png) repeat-x center top / auto 100%;
			width: calc(100% - 20px);
			transition: 0.3s;
			height: 400px;
		}
