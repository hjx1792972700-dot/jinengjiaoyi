...
<template>
	<div class="main-content" style="min-height:100%;width:100%;padding:0;background:none;">
		...
		<template v-if="statType">
			<div class="statShow1">
				<div class="echarts1" v-if="isAuth('xuexiziliao','资料类型统计')">
					<div id="ziliaoleixingChart1" style=" width: 100%;height: 100%"></div>
				</div>
			</div>
		</template>
...
<script>
	...
	export default {
		...
		methods: {
			...
			statChartClick() {
				if(this.isAuth('xuexiziliao','资料类型统计')) {
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					let params = {}
					this.$http({
						url: "xuexiziliao/group/ziliaoleixing",
						method: "get",
						params
					}).then(({data})=>{
						if (data && data.code === 0) {
			...
			getDataList() {
				...
				if(this.searchForm.ziliaobiaoti!='' && this.searchForm.ziliaobiaoti!=undefined){
					params['ziliaobiaoti'] = '%' + this.searchForm.ziliaobiaoti + '%'
				}
