		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('discussxuexiziliao','新增')" type="primary" @click="add('/main/discussxuexiziliaoAdd')">
			...
			<div class="list">
				<el-table class="tables" :stripe='false'
					...
					<el-table-column width="300" label="操作">
						<template #default="scope">
							<el-button class="table-view" type="success" @click="toDetail(scope.row)">
								<span class="icon iconfont icon-fangdajing02"></span>
								查看
							</el-button>
							<el-button class="table-edit" type="success" @click="editClick(scope.row)">
								<span class="icon iconfont icon-xiugai17"></span>
								回复
							</el-button>
							<el-button class="table-del" type="success" @click="delClick(scope.row)" v-if="btnAuth('xuexiziliao','删除评论')">
								<span class="icon iconfont icon-shanchu6"></span>
								删除
							</el-button>
							<el-button class="table-btn1" type="success" @click="discussClick(scope.row)" v-if="btnAuth('discussxuexiziliao','查看评论')">
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				this.$http.get(`discussxuexiziliao/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(async res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/main/discussxuexiziliaoDetail', query: params});
			},
			...
			discussClick (row){
				...
				this.$router.push({path: '/main/discussdiscussxuexiziliao', query: params});
			},
			editClick (row){
				this.$router.push(`/main/discussxuexiziliaoAdd?type=edit&&id=${row.id}`);
			},
			async delClick (row){
				await this.$confirm('是否删除此评论？') .then(_ => {
					this.$http.post('discussxuexiziliao/delete', [row.id]).then(async res => {
						if (res.data.code == 0) {
							await this.$http.get('xuexiziliao/info/' + row.refid).then(async rs=>{
								rs.data.data.discussnum--
								await this.$http.post('xuexiziliao/update',rs.data.data).then(rs2=>{})
							})
							this.$message({
								message: "操作成功",
								type: "success",
			...
			discussistopChange(e,row){
				this.$http.post('discussxuexiziliao/update',row).then(res=>{})
			},
		}
	}
</script>
