<template>
	<div class="fd-page">
				...
				<div class="fd-card">
					<div class="fd-meta">
						<span class="fd-meta-item" v-if="detail.addtime">
							<el-icon><Clock /></el-icon> {{detail.addtime}}
						</span>
						<span class="fd-meta-item">
							<el-icon><User /></el-icon> {{detail.isanon==1&&detail.userid!=userid?'匿名':detail.username}}
						</span>
						<span class="fd-meta-item" v-if="detail.typename">
							<el-icon><CollectionTag /></el-icon> {{detail.typename}}
						</span>
					</div>
					...
					<div class="fd-interact" v-if="detail.id">
						...
						<div class="fd-ibtn" :class="{active: isStore}" @click="storeupClick">
							<el-icon><component :is="isStore ? StarFilled : Star" /></el-icon>
		...
		<el-dialog title="发表评论" v-model="dialogFormVisible" class="fd-dialog" width="500px">
...
<script>
	import { ArrowLeft, Clock, User, CollectionTag, Top, StarFilled, Star, EditPen } from '@element-plus/icons-vue'
	import defaultAvatar from '@/assets/touxiang.png'
	export default {
		components: { ArrowLeft, Clock, User, CollectionTag, Top, StarFilled, Star, EditPen },
		//数据集合
		data() {
			return {
				defaultAvatar,
