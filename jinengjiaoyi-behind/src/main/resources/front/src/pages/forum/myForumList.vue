<template>
	<div class="my-forum-wrap">
		...
		<div class="empty-state" v-if="!loading && tableData.length === 0">
			...
			<button class="empty-btn" type="button" @click="publishPost"><el-icon><EditPen /></el-icon> 写第一篇帖子</button>
		</div>
		...
		<div class="post-list" v-loading="loading" v-if="tableData.length > 0 || loading">
			<div class="post-card" v-for="item in tableData" :key="item.id" @click="toDetail(item)">
				...
				<div class="post-body">
					...
					<div class="post-meta">
						<span class="meta-chip" v-if="item.typename"><el-icon><CollectionTag /></el-icon>{{item.typename}}</span>
						<span class="meta-chip"><el-icon><Calendar /></el-icon>{{item.addtime ? item.addtime.split(' ')[0] : '--'}}</span>
						<span class="meta-chip anon" v-if="item.isanon==1"><el-icon><UserFilled /></el-icon>匿名</span>
						<span class="meta-chip private" v-if="item.isdone==='关闭'"><el-icon><Lock /></el-icon>私人</span>
					</div>
					<div class="post-bottom">
						<div class="post-stats">
							<span><el-icon><Top /></el-icon>{{item.thumbsupnum||0}}</span>
							<span><el-icon><Star /></el-icon>{{item.storeupnum||0}}</span>
						</div>
						<div class="post-actions" @click.stop>
							<button class="act-btn act-view" type="button" @click="toDetail(item)"><el-icon><View /></el-icon>查看</button>
							<button class="act-btn act-edit" type="button" v-if="item.sfsh!=='是'" @click="handleEdit(item)"><el-icon><EditPen /></el-icon>修改</button>
							<button class="act-btn act-del" type="button" @click="handleDelete(item)"><el-icon><Delete /></el-icon>删除</button>
						</div>
					</div>
				</div>
			</div>
