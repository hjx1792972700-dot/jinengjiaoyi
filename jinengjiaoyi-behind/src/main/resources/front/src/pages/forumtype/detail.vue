			...
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						...
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('forumtype','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/main/forumtype', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/main/forumtypeAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此论坛类型？') .then(_ => {
