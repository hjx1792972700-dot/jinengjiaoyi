<template>
	<div :class="{'dark-embedded': embedded}">
		...
		<div class="list-preview">
			<!-- 技能市场主题横幅 -->
			<div class="page-theme-banner skill-banner">
				<div class="ptb-bg">
					<svg viewBox="0 0 1200 80" preserveAspectRatio="none" style="width:100%;height:100%">
						<circle cx="120" cy="40" r="35" fill="rgba(14,165,233,0.06)"/>
						<circle cx="1080" cy="30" r="45" fill="rgba(124,58,237,0.05)"/>
				...
				<div class="ptb-content">
					<div class="ptb-icon">
						<svg viewBox="0 0 36 36" width="32" height="32">
							<circle cx="18" cy="18" r="15" fill="none" stroke="rgba(14,165,233,0.4)" stroke-width="1.5"/>
							<path d="M12 18 L15 15 L19 21 L24 12" fill="none" stroke="#0ea5e9" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
	...
	<div class="sort-bar-wrap">
		<div class="sort-bar-left">
			...
			<div class="sort-tag" :class="{ active: sortType === 'clicknum' }" @click="sortClick('clicknum')">
				<span class="icon iconfont icon-liulan13"></span>
				<span>点击量</span>
				<span class="sort-arrow" v-if="sortType==='clicknum'">{{ sortOrder === 'desc' ? '↓' : '↑' }}</span>
			</div>
			<div class="sort-tag" :class="{ active: sortType === 'storeupnum' }" @click="sortClick('storeupnum')">
				<span class="icon iconfont icon-shoucang10"></span>
				<span>收藏量</span>
				<span class="sort-arrow" v-if="sortType==='storeupnum'">{{ sortOrder === 'desc' ? '↓' : '↑' }}</span>
			</div>
			<div class="sort-tag" :class="{ active: sortType === 'thumbsupnum' }" @click="sortClick('thumbsupnum')">
				<span class="icon iconfont icon-zan10"></span>
				<span>点赞数</span>
				<span class="sort-arrow" v-if="sortType==='thumbsupnum'">{{ sortOrder === 'desc' ? '↓' : '↑' }}</span>
			</div>
			<div class="sort-tag" :class="{ active: sortType === 'totalscore' }" @click="sortClick('totalscore')">
				<span class="icon iconfont icon-kaitongfuwu"></span>
				<span>评分</span>
				<span class="sort-arrow" v-if="sortType==='totalscore'">{{ sortOrder === 'desc' ? '↓' : '↑' }}</span>
			</div>
			<el-button class="list-add-btn" v-if="btnAuth('jinengxuqiu','新增') && centerType" type="primary" size="small" @click="add('/main/jinengxuqiuAdd')">
				<span class="icon iconfont icon-tianjia7"></span> 发布需求
			</el-button>
		</div>
		<div class="sort-bar-right">
			<div class="inline-search">
				<input class="inline-search-input" v-model="formSearch.xuqiubiaoti" placeholder="搜索技能市场..." @keydown.enter="getList(1, curFenlei)" />
				<input class="inline-search-input" v-if="queryChange(['管理员'])" v-model="formSearch.yonghuxingming" placeholder="用户姓名" @keydown.enter="getList(1, curFenlei)" />
				<button class="inline-search-btn" type="button" @click="getList(1, curFenlei)"><el-icon><Search /></el-icon></button>
			</div>
		</div>
	</div>
			<div class="list">
				<div class="list5">
					...
					<div class="imgbox">
						<template v-if="item.fengmian">
							<img v-if="item.fengmian.substr(0,4)=='http'&&item.fengmian.split(',w').length>1" :src="item.fengmian" class="image" />
							<img v-else-if="item.fengmian.substr(0,4)=='http'" :src="item.fengmian.split(',')[0]" class="image" />
							<img v-else :src="baseUrl + item.fengmian.split(',')[0]" class="image" />
						</template>
						<div v-else class="image image-placeholder">
							<el-icon class="placeholder-opportunity-icon"><TrendCharts /></el-icon>
							<span>{{item.jinengfenlei || '技能市场'}}</span>
						</div>
						<div class="card-price" v-if="item.xuqiufeiyong">¥{{item.xuqiufeiyong}}</div>
						<div class="card-price card-price-nego" v-else>面议</div>
					</div>
					<div class="infoBox">
						<div class="card-title">{{item.xuqiubiaoti}}</div>
						<div class="card-meta-row">
							<span class="card-cate">{{item.jinengfenlei}}</span>
							<span class="card-author"><span class="icon iconfont icon-geren16"></span>{{item.yonghuxingming || item.yonghuzhanghao}}</span>
						</div>
						<div class="card-time">
						...
						<div class="card-stats">
							<span class="stat-item" title="点赞"><span class="icon iconfont icon-zan10"></span>{{item.thumbsupnum}}</span>
							<span class="stat-item" title="收藏"><span class="icon iconfont icon-shoucang10"></span>{{item.storeupnum}}</span>
							<span class="stat-item" title="浏览"><span class="icon iconfont icon-chakan2"></span>{{item.clicknum}}</span>
						</div>
					</div>
					</div>
				</div>
		...
		<el-dialog
			...
			<div v-if="changeStatQuery(['users'])" style="display: flex;width: 100%;align-items: center;margin: 0 0 20px;">
				<div style="padding: 0 10px;">技能分类</div>
				<el-select v-model="chartQuery1.jinengfenlei" placeholder="请选择" @change="chartDialog1" clearable>
					<el-option v-for="item in jinengfenleiChartOptions1" :label="item" :value="item"></el-option>
				</el-select>
			</div>
			<div id="jinengfenleiChart1" style="width:100%;height:600px;"></div>
			<template #footer>
	...
	export default {
		...
		methods: {
			...
			if(!this.centerType){
				params['sfsh'] = '是';
				params['leixing'] = '技能';
			} else if(user && user.yonghuzhanghao) {
				searchWhere.yonghuzhanghao = user.yonghuzhanghao;
			}
			if (this.curFenlei != '全部') searchWhere.jinengfenlei = this.curFenlei;
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`jinengxuqiu/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(async res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/main/jinengxuqiuDetail', query: params});
			},
			...
			chartDialogShow1() {
				...
				this.$http.get('option/jinengxuqiu/jinengfenlei',
				).then(rs=>{
					this.jinengfenleiChartOptions1 = rs.data.data
				})
				this.chartDialog1()
			},
			// 统计接口1
			chartDialog1() {
				this.$nextTick(()=>{
					var jinengfenleiChart1 = echarts.init(document.getElementById("jinengfenleiChart1"),'macarons');
					let params = {}
					if(this.chartQuery1.jinengfenlei) {
						params.conditionColumn = 'jinengfenlei'
						params.conditionValue = this.chartQuery1.jinengfenlei
					}
					...
					this.$http.get("jinengxuqiu/group/jinengfenlei",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].jinengfenlei);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].jinengfenlei
								})
							}
							var option = {};
							let titleObj = this.pie.title
							titleObj.text = '技能需求类型'
							...
							// 使用刚指定的配置项和数据显示图表。
							jinengfenleiChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								jinengfenleiChart1.resize();
							};
						}
					})
				})
			...
			chartDialog2() {
				this.$nextTick(()=>{
					...
					this.$http.get("jinengxuqiu/group/fabushijian",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
...
<style rel="stylesheet/scss" lang="scss" scoped>
	.page-theme-banner {
		...
		&.skill-banner { background: linear-gradient(135deg, #0f172a 0%, #1e3a5f 60%, rgba(14,165,233,0.8) 100%); }
	...
	.list-preview {
		...
		.search-bar-wrap {
			...
			.search-input-group {
				...
				&:focus-within {
					border-color: #0ea5e9;
					box-shadow: 0 0 0 3px rgba(149, 119, 65, 0.12);
				}
				.search-icon {
					color: #bbb;
					font-size: 18px;
				...
				.search-main-btn {
					...
					border: none;
					border-radius: 20px;
					background: linear-gradient(135deg, #a8873c, #0ea5e9);
					color: #fff;
					font-size: 14px;
					font-weight: 600;
					padding: 0 24px;
	...
	.sort-bar-wrap {
		...
		.sort-tag {
			...
			&.active {
				...
				color: #7a5f2e;
				font-weight: 600;
				box-shadow: 0 1px 4px rgba(149, 119, 65, 0.15);
				}
			}
		}
		.list {
			...
			.list5 {
				...
				.list-item {
					...
					.imgbox {
						...
						.image-placeholder {
							...
							color: #0ea5e9;
							.placeholder-opportunity-icon { font-size: 36px; margin-bottom: 6px; }
					...
					.infoBox {
						...
						.card-meta-row {
							...
							.card-author {
								font-size: 12px;
								color: #999;
								.icon { font-size: 12px; margin-right: 2px; }
							}
						}
						.card-time {
							font-size: 12px;
							color: #bbb;
							margin-bottom: 10px;
							.icon { font-size: 12px; margin-right: 3px; }
						}
						.card-stats {
							display: flex;
							align-items: center;
				...
				.list-item:hover {
					...
					.imgbox .image { transform: scale(1.05); }
					.infoBox .card-title { color: #0ea5e9; }
				}
			}
		}
	}
...
.dark-embedded {
	...
	.page-theme-banner { display: none !important; }
	...
	.sort-bar-wrap {
		background: transparent !important;
		.sort-label { color: #94a3b8 !important; }
		.sort-tag {
			color: #94a3b8 !important;
			background: rgba(255,255,255,0.03) !important;
			border-color: rgba(255,255,255,0.06) !important;
	...
	.list5 .list-item {
		...
		.image-placeholder {
			background: linear-gradient(135deg, rgba(14,165,233,0.1), rgba(124,58,237,0.08)) !important;
			.placeholder-opportunity-icon { color: #38bdf8 !important; }
			span { color: #64748b !important; }
		}
		.infoBox {
			background: transparent !important;
			.card-title { color: #e2e8f0 !important; }
			.card-meta-row .card-cate {
				background: rgba(14,165,233,0.12) !important;
				color: #38bdf8 !important;
			}
			.card-meta-row .card-author { color: #94a3b8 !important; }
			.card-time { color: #64748b !important; }
			.card-stats {
				border-top-color: rgba(255,255,255,0.06) !important;
				.stat-item { color: #64748b !important; }
			}
		}
		&:hover {
			border-color: rgba(14,165,233,0.2) !important;
			...
			.infoBox .card-title { color: #38bdf8 !important; }
		}
	}
	...
	::v-deep .el-pagination {
		...
		.el-pager li {
			background: rgba(255,255,255,0.04) !important;
			color: #94a3b8 !important;
			&.active { background: linear-gradient(135deg, #0ea5e9, #7c3aed) !important; color: #fff !important; }
		}
		.el-pagination__total { color: #64748b !important; }
		.el-pagination__jump { color: #64748b !important; }
		.el-input__inner {
			background: rgba(255,255,255,0.04) !important;
			border-color: rgba(255,255,255,0.08) !important;
			color: #e2e8f0 !important;
	...
	.list-static-btn {
		...
		color: #94a3b8 !important;
		&:hover { color: #38bdf8 !important; border-color: rgba(14,165,233,0.3) !important; }
	}
}
</style>
