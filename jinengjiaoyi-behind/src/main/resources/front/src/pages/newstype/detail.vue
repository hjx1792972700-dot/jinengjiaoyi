			...
			<div class="attr">
				<div class="info">
					...
					<div class="btn_box">
						...
						<el-button class="editBtn" @click="allchapterClick()" type="warning" v-if="btnAuth('newstype','章节管理')">章节管理</el-button>
					</div>
				</div>
			</div>
		...
		methods: {
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/main/newstype', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/main/newstypeAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此资讯分类？') .then(_ => {
