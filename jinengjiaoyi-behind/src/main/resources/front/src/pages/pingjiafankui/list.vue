<template>
	<div :class="{'dark-embedded': embedded}">
		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				...
				<el-form-item class="list-item" >
					...
					<el-input v-model="formSearch.jinengfenlei" placeholder="技能分类" @keydown.enter="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item" >
					<div class="lable">交换技能：</div>
					<el-input v-model="formSearch.jiaohuanjineng" placeholder="交换技能" @keydown.enter="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item" v-if="queryChange(['管理员'])">
					<div class="lable">交换人姓名：</div>
					<el-input v-model="formSearch.jiaohuanrenxingming" placeholder="交换人姓名" @keydown.enter="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item" >
					<div class="lable">评价结果：</div>
					<el-select v-model="formSearch.pingjiajieguo" placeholder="请选择评价结果" :clearable="true">
						<el-option v-for="(item, index) in pingjiajieguoOptions" :key="index" :label="item" :value="item"></el-option>
					</el-select>
				</el-form-item>
				<div class="list-btn-box">
					...
					<el-button class="list-add-btn" v-if="btnAuth('pingjiafankui','新增')" type="primary" @click="add('/main/pingjiafankuiAdd')">
						<span class="icon iconfont icon-tianjia7"></span>
						新增
					</el-button>
  					<el-button class="list-static-btn" v-if="btnAuth('pingjiafankui','评价统计')" type="success" @click="chartDialogShow1">
				...
				<div class="list5">
					<div v-for="(item,index) in dataList" :key="index" class="list-item" @click.stop="toDetail(item)" >
						<div class="card-inner">
							<div class="imgbox">
								<img v-if="item.huifutupian && item.huifutupian.substr(0,4)=='http'&&item.huifutupian.split(',w').length>1" :src="item.huifutupian" class="image" />
								<img v-else-if="item.huifutupian && item.huifutupian.substr(0,4)=='http'" :src="item.huifutupian.split(',')[0]" class="image" />
								<img v-else :src="baseUrl + (item.huifutupian?item.huifutupian.split(',')[0]:'')" class="image" />
								<div class="card-badge">
									<el-icon><StarFilled /></el-icon> 评价
								</div>
							</div>
							<div class="infoBox">
								<div class="card-title">{{item.biaoti}}</div>
								...
								<div class="bottomInfo">
									...
									<div class="publisher_item">
										...
										<span class="text">{{item.jiaohuanrenxingming || item.jiaohuanrenzhanghao}}</span>
									</div>
								</div>
		...
		<el-dialog
			...
			width="800">
			<div id="pingjiajieguoChart1" style="width:100%;height:600px;"></div>
			<template #footer>
...
<script>
	import { StarFilled, Clock, User, ArrowRight } from '@element-plus/icons-vue';
	import * as echarts from 'echarts'
	import chinaJson from "@/common/echarts/china.json";
	export default {
		components: { StarFilled, Clock, User, ArrowRight },
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				if (this.formSearch.jiaohuanjineng != '') searchWhere.jiaohuanjineng = '%' + this.formSearch.jiaohuanjineng + '%';
				if (this.formSearch.jiaohuanrenxingming != '') searchWhere.jiaohuanrenxingming = '%' + this.formSearch.jiaohuanrenxingming + '%';
				if (this.formSearch.pingjiajieguo != '') searchWhere.pingjiajieguo = this.formSearch.pingjiajieguo;
				if(!this.centerType){
					params['sfsh'] = '是';
				}
				...
				this.$http.get(`pingjiafankui/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(async res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/main/pingjiafankuiDetail', query: params});
			},
			...
			chartDialog1() {
				this.$nextTick(()=>{
					...
					this.$http.get("pingjiafankui/group/pingjiajieguo",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
			...
			chapterClick (row){
				...
				this.$router.push({path: '/main/chapterpingjiafankui', query: params});
			},
		}
	}
</script>
...
.list5 {
	...
	.list-item {
		...
		.card-inner {
			...
			&:hover {
				...
				.card-action {
					...
					i, :deep(.el-icon) { transform: translateX(3px); }
				}
			}
		}
		...
		.card-badge {
			...
			i, :deep(.el-icon) { margin-right: 3px; }
		}
...
.dark-embedded {
	...
	.list5 .list-item .infoBox {
		...
		.bottomInfo {
			...
			i, :deep(.el-icon) { color: #00d4ff !important; }
		}
		.card-action {
