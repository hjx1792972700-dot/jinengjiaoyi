...
<template>
	<div class="add-update-preview">
		<el-form
			...
			label-width="200px"
			>
			<el-form-item class="add-item" label="资料标题" prop="ziliaobiaoti">
				<el-input v-model="ruleForm.ziliaobiaoti" 
					placeholder="资料标题" clearable :readonly="ro.ziliaobiaoti"></el-input>
			</el-form-item>
			<el-form-item class="add-item"  label="资料类型" prop="ziliaoleixing">
				<el-select v-model="ruleForm.ziliaoleixing" placeholder="请选择资料类型" :disabled="ro.ziliaoleixing"  filterable>
					<el-option
			...
			<el-form-item class="add-item" label="资料封面" v-if="type!='cross' || (type=='cross' && !ro.ziliaofengmian)" prop="ziliaofengmian">
				<file-upload
					tip="点击上传资料封面"
					action="file/upload"
					:limit="3"
			...
			<el-form-item class="add-item" v-else label="资料封面" prop="ziliaofengmian">
				<img v-if="ruleForm.ziliaofengmian.substring(0,4)=='http'" class="upload-img" v-bind:key="index" :src="ruleForm.ziliaofengmian.split(',')[0]">
				<img v-else class="upload-img" v-bind:key="index" v-for="(item,index) in ruleForm.ziliaofengmian.split(',')" :src="baseUrl+item">
			</el-form-item>
			<el-form-item class="add-item" label="资料视频" v-if="type!='cross' || (type=='cross' && !ro.ziliaoshipin)" prop="ziliaoshipin">
				<file-upload
					tip="点击上传资料视频"
					action="file/upload"
					:limit="1"
			...
			<el-form-item class="add-item" v-else label="资料视频" prop="ziliaoshipin">
				<div class="viewBtn" v-if="ruleForm.ziliaoshipin" @click="download(baseUrl + ruleForm.ziliaoshipin )" type="success">预览</div>
				<div class="unviewBtn" v-else disabled>暂无</div>
			</el-form-item>
			<el-form-item class="add-item" label="资料附件" prop="ziliaofujian">
				<file-upload
					tip="点击上传资料附件"
					action="file/upload"
					:limit="1"
...
<script>
	export default {
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='ziliaofengmian'){
							this.ruleForm.ziliaofengmian = obj[o]?obj[o].split(",")[0]:'';
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`xuexiziliao/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			ziliaofengmianUploadChange(fileUrls) {
				this.ruleForm.ziliaofengmian = fileUrls.replace(/^\/+/, "");
			},
			ziliaoshipinUploadChange(fileUrls) {
				this.ruleForm.ziliaoshipin = fileUrls.replace(/^\/+/, "");
			},
			ziliaofujianUploadChange(fileUrls) {
				this.ruleForm.ziliaofujian = fileUrls.replace(/^\/+/, "");
			},
		}
	};
</script>
