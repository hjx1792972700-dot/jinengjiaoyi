...
<template>
	<div class="add-update-preview">
		<el-form
			...
			label-width="200px"
			>
			<el-form-item class="add-item" label="提问" prop="ask">
				<el-input v-model="ruleForm.ask" 
					placeholder="提问" clearable :readonly="ro.ask"></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="回复" prop="reply">
				<el-input
					type="textarea"
					:rows="8"
					:disabled="ro.reply"
...
<script>
	export default {
		...
		methods: {
			...
			// 多级联动参数
			async info() {
				await this.$http.get(`chathelper/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit(subMitType=null) {
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`chathelper/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								await this.$message({
									message: '操作成功',
									type: 'success',
