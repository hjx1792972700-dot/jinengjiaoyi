		...
		<div class="detail-card">
			...
			<div class="card-info">
				<div class="title-row">
					...
					<span class="price-tag" v-if="detail.xuqiufeiyong"><i class="el-icon-coin"></i> ¥{{detail.xuqiufeiyong}}</span>
					<span class="price-tag negotiate" v-else><i class="el-icon-coin"></i> 面议</span>
				...
				<div class="meta-tags">
					<span class="tag"><i class="el-icon-document"></i> {{detail.xuqiubianhao || '--'}}</span>
					<span class="tag"><i class="el-icon-collection-tag"></i> {{detail.jinengfenlei || '--'}}</span>
					<span class="tag"><i class="el-icon-date"></i> {{detail.fabushijian || '--'}}</span>
					<span class="tag"><i class="el-icon-user"></i> {{detail.yonghuxingming || '--'}}</span>
					<span class="tag"><i class="el-icon-phone"></i> {{detail.shoujihao || '--'}}</span>
					<span class="tag"><i class="el-icon-medal"></i> 信誉 {{detail.xinyuzhishu || '--'}}</span>
					<span class="tag"><i class="el-icon-view"></i> {{detail.clicknum || '0'}}次浏览</span>
					<span class="tag"><i class="el-icon-star-on"></i> {{detail.totalscore || '0'}}分</span>
					<span class="tag status-tag pass" v-if="centerType && detail.sfsh=='是'"><i class="el-icon-circle-check"></i> 已通过</span>
					<span class="tag status-tag fail" v-if="centerType && detail.sfsh=='否'"><i class="el-icon-circle-close"></i> 未通过</span>
					<span class="tag status-tag pending" v-if="centerType && (detail.sfsh=='待审核'||!detail.sfsh)"><i class="el-icon-time"></i> 待审核</span>
				</div>
					...
					<el-form class="comment-inline-form" :model="form" :rules="rules" ref="form">
						...
						<div class="comment-action-row">
							<div class="rate-inline">
								<span class="rate-label"><i class="el-icon-star-on"></i> 评分</span>
								<el-rate
									v-model="form.score"
									:max='Number(5)'
									:allow-half='false'
							...
							<div class="comment-btns">
								<el-button class="submit-btn" size="small" @click="submitForm('form')"><i class="el-icon-promotion"></i> 提交评论</el-button>
					...
					<div v-if="infoList.length" class="comment-list">
						<div class="comment-item" v-for="item in infoList" :key="item.id" @mouseenter="discussEnter(item.id)" :id="'discuss' + item.id"
							...
							<div class="comment-reply-box" v-if="item.reply">
								<i class="el-icon-chat-dot-round"></i> 回复：<span class="ql-snow ql-editor" v-html="item.reply"></span>
							</div>
						</div>
					</div>
