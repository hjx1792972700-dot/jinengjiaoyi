	...
	<div class="apply-detail-page">
		<div class="page-header">
			...
			<div class="header-right">
				<span class="status-badge good" v-if="detail.pingjiajieguo=='好评'"><i class="el-icon-star-on"></i> 好评</span>
				<span class="status-badge bad" v-else-if="detail.pingjiajieguo=='差评'"><i class="el-icon-star-off"></i> 差评</span>
				<span class="status-badge normal" v-else-if="detail.pingjiajieguo=='一般'"><i class="el-icon-minus"></i> 一般</span>
				<span class="status-badge audit-pass" v-if="centerType && detail.sfsh=='是'"><i class="el-icon-circle-check"></i> 已通过</span>
				<span class="status-badge audit-fail" v-if="centerType && detail.sfsh=='否'"><i class="el-icon-circle-close"></i> 未通过</span>
				<span class="status-badge audit-pending" v-if="centerType && (detail.sfsh=='待审核'||!detail.sfsh)"><i class="el-icon-time"></i> 待审核</span>
		...
		<div class="exchange-flow">
			<div class="flow-card applicant-card">
				<div class="card-label"><i class="el-icon-edit"></i> 评价人</div>
			...
			<div class="flow-card owner-card">
				<div class="card-label"><i class="el-icon-user"></i> 被评价人</div>
		...
		<div class="detail-bottom">
			...
			<div class="remark-card audit-card" v-if="centerType && detail.shhf">
				<div class="remark-icon audit"><i class="el-icon-document"></i></div>
				<div class="remark-body">
			...
			<div class="media-row" v-if="detailBanner.length > 1 || detail.huifushipin || detail.huifufujian">
				...
				<div class="media-item" v-if="detail.huifufujian">
					<el-button class="dl-btn" size="small" icon="el-icon-download" @click="download(detail.huifufujian)">下载附件</el-button>
				</div>
			</div>

			<div class="reply-form-card">
				<div class="reply-form-title"><i class="el-icon-edit"></i> {{detail.huifuneirong ? '追加回复' : '回复评价'}}</div>
				<el-input
					type="textarea"
					:rows="3"
				...
				<div class="reply-form-actions">
					<el-button class="reply-submit-btn" size="small" icon="el-icon-promotion" @click="submitReply" :loading="replyLoading">提交回复</el-button>
				</div>
			</div>

			<div class="bottom-row">
				<div class="time-info">
					<i class="el-icon-time"></i> 评价时间：{{detail.pingjiashijian || '--'}}
				</div>
