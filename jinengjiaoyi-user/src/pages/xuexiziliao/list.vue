<template>
	<div>
		...
		<div class="list-preview">
			<!-- 学习资料主题横幅 -->
			<div class="page-theme-banner material-banner">
				<div class="ptb-bg">
					<svg viewBox="0 0 1200 80" preserveAspectRatio="none" style="width:100%;height:100%">
						<circle cx="100" cy="40" r="30" fill="rgba(124,58,237,0.06)"/>
						<circle cx="1100" cy="35" r="40" fill="rgba(14,165,233,0.05)"/>
						<path d="M0,40 Q300,15 600,40 T1200,40" fill="none" stroke="rgba(124,58,237,0.1)" stroke-width="1.5"/>
					</svg>
				</div>
				<div class="ptb-content">
					<div class="ptb-icon">
						<svg viewBox="0 0 36 36" width="32" height="32">
							<rect x="6" y="4" width="24" height="28" rx="3" fill="none" stroke="rgba(124,58,237,0.4)" stroke-width="1.5"/>
							<line x1="11" y1="12" x2="25" y2="12" stroke="#7c3aed" stroke-width="1.5" stroke-linecap="round"/>
							<line x1="11" y1="17" x2="22" y2="17" stroke="#0ea5e9" stroke-width="1.5" stroke-linecap="round"/>
							<line x1="11" y1="22" x2="20" y2="22" stroke="#7c3aed" stroke-width="1.5" stroke-linecap="round" opacity="0.6"/>
						</svg>
					</div>
					<div class="ptb-text">
						<h3>学习资料</h3>
		...
		<div class="sort-bar-wrap">
			<div class="sort-bar-left">
				...
				<el-button class="list-add-btn" v-if="btnAuth('xuexiziliao','新增')" type="primary" size="small" @click="add('/main/xuexiziliaoAdd')">
					<span class="icon iconfont icon-tianjia7"></span> 发布资料
				</el-button>
				<el-button class="list-add-btn list-stat-btn" v-if="btnAuth('xuexiziliao','资料类型统计')" type="primary" size="small" @click="chartDialogShow1">
					资料统计
				</el-button>
			</div>
		...
		<div class="list">
			<div class="list5">
				<div v-for="(item,index) in dataList" :key="index" class="list-item" @click.stop="toDetail(item)" >
					<div class="imgbox">
						<img v-if="item.ziliaofengmian && item.ziliaofengmian.substr(0,4)=='http'&&item.ziliaofengmian.split(',w').length>1" :src="item.ziliaofengmian" class="image" />
						<img v-else-if="item.ziliaofengmian && item.ziliaofengmian.substr(0,4)=='http'" :src="item.ziliaofengmian.split(',')[0]" class="image" />
						<img v-else :src="baseUrl + (item.ziliaofengmian?item.ziliaofengmian.split(',')[0]:'')" class="image" />
						<div class="card-tag" v-if="item.ziliaoleixing">{{item.ziliaoleixing}}</div>
						<div class="price-badge" v-if="item.jiage > 0">&#165;{{item.jiage}}</div>
						<div class="price-badge free" v-else>免费</div>
					</div>
					<div class="infoBox">
						<div class="name">{{item.ziliaobiaoti}}</div>
						<div class="card-meta">
							<span class="meta-time"><i class="el-icon-time"></i>{{item.addtime?item.addtime.split(' ')[0]:''}}</span>
						</div>
		...
		<el-dialog
			...
			width="800">
			<div id="ziliaoleixingChart1" style="width:100%;height:600px;"></div>
			<template #footer>
	...
	export default {
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				let searchWhere = {};
				if (this.formSearch.ziliaobiaoti != '') searchWhere.ziliaobiaoti = '%' + this.formSearch.ziliaobiaoti + '%';
				if (this.formSearch.ziliaoleixing != '') searchWhere.ziliaoleixing = this.formSearch.ziliaoleixing;
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`xuexiziliao/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(async res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/main/xuexiziliaoDetail', query: params});
			},
			...
			chartDialog1() {
				this.$nextTick(()=>{
					var ziliaoleixingChart1 = echarts.init(document.getElementById("ziliaoleixingChart1"),'macarons');
					let params = {}
					this.$http.get("xuexiziliao/group/ziliaoleixing",{params:params}).then(({data})=>{
						if (data && data.code === 0) {
							let res = data.data;
							let xAxis = [];
							let yAxis = [];
							...
							for(let i=0;i<res.length;i++){
								...
								xAxis.push(res[i].ziliaoleixing);
								yAxis.push(parseFloat((res[i].total)));
								pArray.push({
									value: parseFloat((res[i].total)),
									name: res[i].ziliaoleixing
								})
							}
							var option = {};
							let titleObj = this.pie2.title
							titleObj.text = '资料类型统计'
							...
							// 使用刚指定的配置项和数据显示图表。
							ziliaoleixingChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								ziliaoleixingChart1.resize();
							};
						}
					})
				})
...
<style rel="stylesheet/scss" lang="scss" scoped>
	.page-theme-banner {
		...
		&.material-banner { background: linear-gradient(135deg, #0f172a 0%, #2d1b69 60%, rgba(124,58,237,0.8) 100%); }
	...
	.list-preview {
		...
		.sort-tag {
			...
			white-space: nowrap;
			&:hover { color: #0ea5e9; border-color: #0ea5e9; }
			&.active {
				color: #0ea5e9;
		...
		.list-stat-btn {
			...
			&:hover { color: #0ea5e9 !important; border-color: #0ea5e9 !important; }
		}
		.inline-search {
			display: flex;
			align-items: center;
				...
				.infoBox {
					...
					.card-meta {
						...
						.meta-time {
							font-size: 12px;
							color: #94a3b8;
							.el-icon { margin-right: 4px; font-size: 13px; vertical-align: middle; }
						}
					}
					.card-stats {
						display: flex;
		...
		@media (max-width: 1100px) {
			.list5 .list-item { width: calc(33.33% - 20px); }
		}
		@media (max-width: 768px) {
			.list5 .list-item { width: calc(50% - 20px); }
		}
		}
	}
	.no-exchange-notice {
